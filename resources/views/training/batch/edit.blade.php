    ...
    <div class="row">
        <div class="col-md-12">
            <div class="box">
                ...
                <!-- /.box-header -->
                <!-- form start -->
                <form class="form-horizontal" method="POST" action="{{ route('batch_edit', ['batch' => $batch->id]) }}">
                    @csrf
                    ...
                    <div class="box-body">
                            ...
                            <div class="col-sm-10">
                                <select class="form-control" name="course">
                                    ...
                                    @foreach($courses as $course)
                                        <option value="{{ $course->id }}" {{ empty(old('course')) ? ($errors->has('course') ? '' : ($batch->course_id == $course->id ? 'selected' : '')) :
                        ...
                        <div class="form-group {{ $errors->has('batch_code') ? 'has-error' :'' }}">
                            ...
                            <div class="col-sm-10">
                                <input type="text" class="form-control" placeholder="Enter Batch Code"
                                       name="batch_code" value="{{ empty(old('batch_code')) ? ($errors->has('batch_code') ? '' : $batch->batch_code) : old('batch_code') }}">
                        ...
                        <div class="form-group {{ $errors->has('batch_amount') ? 'has-error' :'' }}">
                            ...
                            <div class="col-sm-10">
                                <input type="number" class="form-control" placeholder="Enter Batch Amount"
                                       name="batch_amount" value="{{ empty(old('batch_amount')) ? ($errors->has('batch_amount') ? '' : $batch->batch_amount) : old('batch_amount') }}">
                        ...
                        <div class="form-group {{ $errors->has('total_mark') ? 'has-error' :'' }}">
                            ...
                            <div class="col-sm-10">
                                <input type="number" class="form-control" placeholder="Enter Total Mark"
                                       name="total_mark" value="{{ empty(old('total_mark')) ? ($errors->has('total_mark') ? '' : $batch->total_mark) : old('total_mark') }}">
...
{{--                                    {{dd($batch->id)}}--}}
                                    @foreach($batch->batchLevels as $batchLevel)
                                    <tr class="level-item">
                                        <td>
                                            <input type="text" step="any" class="form-control level" value="{{ empty(old('level')) ? ($errors->has('level') ? '' : $batchLevel->level) : old('level') }}" name="level[]">
                                        </td>
                                        <td>
                                            <input type="number" step="any" class="form-control mark"  name="mark[]" value="{{ empty(old('mark')) ? ($errors->has('mark') ? '' : $batchLevel->mark) : old('mark') }}">
                                        </td>
                        ...
                        <div class="form-group {{ $errors->has('sort') ? 'has-error' :'' }}">
                            ...
                            <div class="col-sm-10">
                                <input type="number" value="{{ empty(old('sort')) ? ($errors->has('sort') ? '' : $batch->sort) : old('sort') }}"
