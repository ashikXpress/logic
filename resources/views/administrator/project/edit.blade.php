        ...
        <div class="col-md-12">
            <div class="box">
                ...
                <form class="form-horizontal"
                      method="POST"
                      action="{{ route('project.edit', ['project' => $project->id]) }}" enctype="multipart/form-data">
                    @csrf
                            ...
                            <div class="col-sm-10">
                                <select class="form-control" name="sister_concern" id="sister_concern">
                                    ...
                                    @foreach($sisterConcerns as $sisterConcern)
                                        <option value="{{ $sisterConcern->id }}" {{ empty(old('sister_concern')) ? ($errors->has('sister_concern') ? '' : ($project->sister_concern_id == $sisterConcern->id ? 'selected' : '')) :
                        ...
                        <div class="form-group {{ $errors->has('description') ? 'has-error' :'' }}">
                            ...
                            <div class="col-sm-10">
                                <textarea class="form-control"
                                          ...
                                          name="description">{{ empty(old('description')) ? ($errors->has('description') ? '' : $project->description) : old('description') }}</textarea>
                        ...
                        <div class="form-group {{ $errors->has('deadline') ? 'has-error' :'' }}">
                            ...
                            <div class="col-sm-10">
                                <div class="input-group date">
                                    ...
                                    <input type="text" class="form-control pull-right date" name="deadline" value="{{ empty(old('deadline')) ? ($errors->has('deadline') ? '' : $project->deadline->format('Y-m-d')) : old('deadline') }}" autocomplete="off">
                                </div>
                                <!-- /.input group -->
...
@section('script')
    ...
    <script>
        $(function () {
            ...
            var clientSelected = '{{ empty(old('client')) ? ($errors->has('client') ? '' : $project->client_id) : old('client') }}';
