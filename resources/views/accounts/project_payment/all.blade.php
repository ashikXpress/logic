            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="modal-form" enctype="multipart/form-data" name="modal-form">
                        ...
                        <div class="form-group">
                            <label>Project</label>
                            <select class="form-control" id="modal-project" name="project">
                                <option value="">Select Project</option>
                            </select>
                        </div>

                        <div id="modal-project-info" style="background-color: lightgrey; padding: 10px; border-radius: 3px;"></div>
    ...
    <script>
        $(function () {
            $('#table').DataTable({
                processing: true,
                serverSide: true,
                ajax: '{{ route('project_payment.datatable') }}',
                columns: [
                    {data: 'sisterConcern', name: 'sisterConcern.name'},
                    {data: 'name', name: 'name', orderable: false},
                    {data: 'total', name: 'total', orderable: false},
                    {data: 'receive', name: 'receive', orderable: false},
            ...
            $('body').on('click', '.btn-pay', function () {
                ...
                $('#modal-project').html('<option value="">Select Project</option>');
                $('#modal-project-info').hide();
                $('#modal-name').val(clientName);

                $.ajax({
                    method: "GET",
                    url: "{{ route('project_payment.get_projects') }}",
                    data: { clientId: clientId }
                }).done(function( response ) {
                    $.each(response, function( index, item ) {
                        $('#modal-project').append('<option value="'+item.id+'">'+item.name+'</option>');
                    });
            ...
            $('#modal-btn-pay').click(function () {
                ...
                $.ajax({
                    type: "POST",
                    url: "{{ route('project_payment.make_payment') }}",
                    data: formData,
                    processData: false,
                    contentType: false,
            ...
            $('#modal-project').change(function () {
                ...
                if (projectId != '') {
                    $.ajax({
                        method: "GET",
                        url: "{{ route('get_project_details') }}",
                        data: { projectId: projectId }
                    }).done(function( response ) {
                        $('#modal-project-info').html('<strong>Total: </strong>৳'+parseFloat(response.amount).toFixed(2)+' <strong>Receive: </strong>৳'+parseFloat(response.receive).toFixed(2)+' <strong>Due: </strong>৳'+parseFloat(response.due).toFixed(2));
